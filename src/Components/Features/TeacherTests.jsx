...
import {
  ...
  XMarkIcon,
  Bars3Icon,
  EyeIcon, // For View Attempts
  UsersIcon, // For attendees count
...
import {
  ...
  where,
  doc,
  // updateDoc, // If editing tests is needed later
  // deleteDoc, // If deleting tests is needed later
  serverTimestamp,
  orderBy
...
const TeacherTests = () => {
    ...
    const [createdTests, setCreatedTests] = useState([]); // Will be fetched from Firebase
    const [loading, setLoading] = useState(false); // For Gemini and Firebase operations
    const [error, setError] = useState('');

    // For viewing submissions
    const [showSubmissionsModal, setShowSubmissionsModal] = useState(false);
    const [viewingTestSubmissions, setViewingTestSubmissions] = useState(null); // Test object
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, (user) => {
            if (user) {
                ...
                setCurrentUser(null);
                setCreatedTests([]); // Clear tests if user logs out
    ...
    const handleSaveAndPublishTest = async () => {
        ...
        try {
            const testToSave = {
                ...
                topics: newTest.topics,
                difficulty: newTest.difficulty,
                numQuestions: newTest.questions.length, // Ensure this is accurate
            ...
            // Add to local state for immediate UI update, or refetch
            setCreatedTests(prev => [{ id: docRef.id, ...testToSave, createdAt: new Date() }, ...prev]); // Approximate createdAt for UI
        ...
        <div className="font-sans min-h-screen bg-slate-900 flex text-slate-100">
                ...
                <div className="relative flex flex-col w-72 max-w-xs h-full bg-slate-800">
                    ...
                    <nav className="flex-1 overflow-y-auto p-4 space-y-1">
                        {educatorSidebarMenu.map((item) => (
                            <a key={item.title} href={item.link} onClick={(e) => { if (item.link === '/teacher-tests') { e.preventDefault(); setCurrentView('dashboard'); setSidebarOpen(false); }}}
                                className={`flex items-center px-4 py-3 rounded-lg text-sm font-medium transition-colors duration-200 ${ item.title === 'Tests' && (currentView === 'dashboard' || currentView === 'createTest' || currentView === 'review') ? 'bg-indigo-700 text-white' : 'text-slate-300 hover:bg-slate-700/50 hover:text-white'} ${item.special ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white hover:opacity-90' : ''}`}>
            ...
            <div className="hidden lg:flex lg:flex-shrink-0">
                <div className="flex flex-col w-72 border-r border-slate-700 bg-slate-800">
                    ...
                    <nav className="flex-1 overflow-y-auto p-4 space-y-1">
                        {educatorSidebarMenu.map((item) => (
                            <a key={item.title} href={item.link} onClick={(e) => { if (item.link === '/teacher-tests') { e.preventDefault(); setCurrentView('dashboard'); }}}
                                className={`flex items-center px-4 py-3 rounded-lg text-sm font-medium transition-colors duration-200 ${ item.title === 'Tests' && (currentView === 'dashboard' || currentView === 'createTest' || currentView === 'review') ? 'bg-indigo-700 text-white' : 'text-slate-300 hover:bg-slate-700/50 hover:text-white'} ${item.special ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white hover:opacity-90' : ''}`}>
                                <item.Icon className="flex-shrink-0 h-5 w-5 mr-3" /> {item.title}
                                {item.special && (<span className="ml-auto px-2 py-0.5 text-xs rounded-full bg-white/20">PRO</span>)}
                            </a>
            ...
            <div className="flex-1 overflow-x-hidden overflow-y-auto">
                <div className="lg:hidden sticky top-0 z-30 bg-slate-800/80 backdrop-blur-md p-4 border-b border-slate-700 flex items-center justify-between">
                    ...
                    <h1 className="text-xl font-bold text-white"> {currentView === 'createTest' ? 'Create Test' : currentView === 'review' ? 'Review Questions' : 'Tests Dashboard'} </h1>
                    <div></div>
                </div>
